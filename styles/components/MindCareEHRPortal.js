...
Continue weekly sessions. Assign daily anxiety tracking worksheet. Practice relaxation techniques twice daily. Next appointment: ${new Date(Date.now() + 7*24*60*60*1000).toLocaleDateString()}`;
      ...
      if (typeof window !== 'undefined') {
        window.alert('Recording complete! Session transcribed successfully.');
      }
  ...
  const generateNotes = () => {
    if (!sessionInput || !selectedClient || !selectedService) {
      if (typeof window !== 'undefined') {
        window.alert('Please select client, service, and provide session notes');
      }
      return;
    }
...
Date: ${new Date().toLocaleDateString()}`;
  ...
  const signNotes = () => {
    if (!generatedNotes) {
      if (typeof window !== 'undefined') {
        window.alert('Please generate notes first');
      }
      return;
    }
    ...
    if (typeof window !== 'undefined') {
      window.alert(`Notes digitally signed and locked at ${timestamp}`);
    }
  };

  const downloadNotes = () => {
    if (!generatedNotes || typeof window === 'undefined') return;
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
              ...
              {gmailConnected && (
                <button
                  onClick={() => {
                    if (typeof window !== 'undefined') {
                      window.alert('Compose new secure email - Demo mode');
                    }
                  }}
                  style={{
              ...
              <div style={{ 
                ...
                <div>
                  ...
                  >
                    ...
                    {clients.map(client => (
                      <option key={client.id} value={client.id.toString()}>{client.firstName} {client.lastName}</option>
                    ))}
                  </select>
                </div>
          ...
          <div>
            ...
            {!calendarConnected ? (
              <div style={{
                ...
                <p style={{ color: '#64748b', marginBottom: '2rem' }}>
                  Enable appointment scheduling and management with Google Calendar integration
                </p>
                <button onClick={connectGoogleCalendar} style={{
                  padding: '1rem 2rem',
                  backgroundColor: '#2563eb',
              ...
              <div style={{
                ...
                <h3 style={{ marginBottom: '2rem', color: '#374151' }}>Upcoming Appointments</h3>
                {appointments.map(appointment => (
                  <div key={appointment.id} style={{
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <div>
                        <h4 style={{ margin: 0, color: '#1e293b' }}>{appointment.clientName}</h4>
          ...
          <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '2rem' }}>
              <div style={{
                ...
                <h3 style={{ marginBottom: '1rem', color: '#1e293b' }}>📧 Gmail Integration</h3>
                <p style={{ color: '#64748b', marginBottom: '1rem', fontSize: '0.875rem' }}>
                  Secure HIPAA-compliant email communication with clients
                </p>
                <div style={{ marginBottom: '1rem' }}>
                  <div style={{ fontSize: '0.875rem', color: '#374151' }}>
                    Status: {gmailConnected ? (
                      ...
                      <span style={{ color: '#dc2626', fontWeight: '600' }}>Not Connected</span>
                    )}
                  </div>
                </div>
                {!gmailConnected && (
              ...
              <div style={{
                ...
                <h3 style={{ marginBottom: '1rem', color: '#1e293b' }}>📅 Google Calendar</h3>
                <p style={{ color: '#64748b', marginBottom: '1rem', fontSize: '0.875rem' }}>
              ...
              <div style={{
                ...
                <h3 style={{ marginBottom: '1rem', color: '#1e293b' }}>🎥 Google Meet</h3>
                <p style={{ color: '#64748b', marginBottom: '1rem', fontSize: '0.875rem' }}>
                  Secure telehealth video sessions
                </p>
                <div style={{ marginBottom: '1rem' }}>
                  <div style={{ fontSize: '0.875rem', color: '#374151' }}>
                    Status: <span style={{ color: '#059669', fontWeight: '600' }}>✓ Available</span>
                  </div>
                </div>
                <button 
                  onClick={() => {
                    if (typeof window !== 'undefined') {
                      window.alert('Google Meet integration ready! Connects automatically with calendar appointments.');
                    }
                  }}
                  style={{
                    width: '100%',
